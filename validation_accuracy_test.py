...
testing_transformations = trans.Compose([trans.ToTensor(), 
                                 trans.Normalize(mean=mean, std=std),
                                 trans.Resize((100,100))])
...
testing_dataset = torchvision.datasets.Flowers102(root='./data', split="test", 
                                                   download=True, transform=testing_transformations)
...
def TestDataNetworkAndClassesAccuracy():
    """ Calculates and prints the accuracy of the network as a whole AND of each class prediction using test data split"""
    neural_net.eval()
    with torch.no_grad():
        n_correct = 0
        n_samples = 0
...
def ValidationDataNetworkAccuracyOnly():
    """Calculates and prints ONLY the accuracy of the network as a whole - NOT of each class using the validation data split
    ...
    with torch.no_grad():
        ...
        acc = 100.0 * n_correct / n_samples
        print(f'Accuracy of the network on Validation dataset: {acc} %')
        neural_net.train()
        return acc
...
for ep in range(epochs): # Each iteration is a forward pass to train the data
    ...
    current_accuracy = ValidationDataNetworkAccuracyOnly()
...
# print("Network Accuracy on Testing Dataset After Training:")
# neural_net.eval()
# TestDataNetworkAndClassesAccuracy()