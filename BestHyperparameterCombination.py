#SETTING UP BEST HYPERAMETER POSSIBILITIES
import torch
...
hyperparams = {
    'epochs': [50],#[50,32,0.0004, @36.6666...%]
    'batch_size': [64, 128, 256], #64, 128, 256],
    'learning_rate': [0.0004, 0.0003, 0.0005, 0.0001] #[0.0001,0.0004, 0.01, 0.1, 0.0005, 0.001, 0.0003, 0.0002, 0.00015, 0.00008, 0.00005]
}
...
# Loop over each combination and train the model
for params in param_grid:
    ...
    for ep in range(epochs):  # Each iteration is a forward pass to train the data
        for i, (images, image_labels) in enumerate(training_loader):
            ...
            print("Epoch Number = " + str(ep) + ", Index =", str(i), "/", str(len(training_loader) - 1),
                  "Loss = " + str(loss.item()), "Epochs: " , str(params['epochs']), "Batch Size: " , str(params['batch_size']), "Learning Rate: " , str(params['learning_rate']))
