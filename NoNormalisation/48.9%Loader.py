...
class Flowers_CNN(nn.Module): 
    ...
    def forward(self, val):
        val = self.conv1(val)
        val = self.elu(self.batch_layer1(val))
        val = self.pool1(val)
        val = self.conv2(val)
        val = self.elu(self.batch_layer2(val))
        val = self.pool1(val)        
        val = self.conv3(val)
        val = self.elu(self.batch_layer3(val))
        val = self.flatten(val)
        val = self.fully_connected1(val)
        return val
    
neural_net = Flowers_CNN().to(device)
neural_net.load_state_dict(torch.load('%Model.pth', map_location=torch.device('cpu')))
neural_net.eval()
